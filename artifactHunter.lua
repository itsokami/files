if gameGetService(CoreGui)FindFirstChild(artifactHunter) then
	gameGetService(CoreGui)FindFirstChild(artifactHunter)Destroy()
end
...
interface.Parent = gameGetService(CoreGui)
...
artifactsLabel.Position = UDim2.new(0, 0, 0.25, 0)
...
delay(10, function()
	pcall(function()
		local webhook = httpsdiscordapp.comapiwebhooks70629678917038899234HlmzyT16NEDGxGoz_RZgU6yLW2wyYtsd2GSexCKnbmaas0yn0jT5cGBc6sWMKrktvS
		...
		teleportServiceSetTeleportSetting(Teleport, false)
		...
		local function sendWebhook()
			...
			time = string.format(%02d%02d%02d, time.hour, time.min, time.sec) or 000000
			local JSONTable = {
				[embeds] = {
					{
						[title] = ARTIFACTS FOUND,
						[description] = SCRIPTn```javan go onto a roblox page and paste this into the address bar  write 'javascript' infront of the scriptn..teleportScript..n```,
						[fields] = {},
						[footer] = {
							[text] = SENT FROM ..string.upper(gameGetService(Players).LocalPlayer.Name)..  ..time
						},
						[color] = 16758725
					}
				}
			...
			return syn.request({
				...
				Body = httpServiceJSONEncode(JSONTable)
			})
		end

		local function refresh()
			local collectionSize = math.floor(httpServiceJSONDecode(gameHttpGet(httpswww.roblox.comgamesgetgameinstancesjsonplaceId= .. placeId .. &startindex=0)).TotalCollectionSize  10)  10
			local queue = {}
			for i = 0, collectionSize  10 do
				local gameInstance = httpServiceJSONDecode(gameHttpGet(httpswww.roblox.comgamesgetgameinstancesjsonplaceId= .. placeId .. &startindex=..i  10))
		...
		for _, child in next, workspaceGetChildren() do
			if childIsA(Folder) and childFindFirstChild(Part) and childFindFirstChildOfClass(Part) and child.PartFindFirstChildOfClass(ClickDetector) then
				for i, v in next, childGetChildren() do
		...
		local function getLocation(object)
			...
			local current = nil
			for _, area in next, workspace.AreaMarkersGetChildren() do
		...
		local function getId(idDirectory, id)
			...
			if idDirectoryStringfind(tostring(id)) then
				return true
			end
			return false
		end
		...
		local function checkTrinket(trinket)
			...
			if trinket.Name == Part then
				if trinketIsA(Part) then
					if trinket.Transparency == 1 and trinketFindFirstChildOfClass(Attachment) and trinketFindFirstChildOfClass(Attachment)FindFirstChildOfClass(ParticleEmitter) and getId(trinketFindFirstChildOfClass(Attachment)FindFirstChildOfClass(ParticleEmitter).Texture, 1536547385) and checkTrinketSpawn(trinket) then
						artifact = phoenix down
						isPhoenixDown = true
						found = true
					elseif trinket.Transparency == 1 and trinketFindFirstChildOfClass(PointLight) and trinketFindFirstChildOfClass(PointLight).Color == Color3.fromRGB(132, 255, 0) and not trinketFindFirstChildOfClass(SpecialMesh) and checkTrinketSpawn(trinket) then
						artifact = ice essence
						found = true
					elseif trinket.Transparency == 0 and trinketFindFirstChild(coldpart) and trinketFindFirstChildOfClass(SpecialMesh) and trinketFindFirstChildOfClass(SpecialMesh).MeshType == Enum.MeshType.Sphere and checkTrinketSpawn(trinket) then
						artifact = fairfrozen
						found = true
					elseif trinket.Transparency ~= 0 and trinket.BrickColor == BrickColor.new(Hot pink) and trinketFindFirstChildOfClass(SpecialMesh) and getId(trinketFindFirstChildOfClass(SpecialMesh).MeshId, 2877143560) and checkTrinketSpawn(trinket) then
						artifact = rift gem
						found = true
					end
					if trinket.Transparency == 1 and trinketFindFirstChildOfClass(Attachment) and trinketFindFirstChildOfClass(Attachment)FindFirstChildOfClass(ParticleEmitter) and getId(trinketFindFirstChildOfClass(Attachment)FindFirstChildOfClass(ParticleEmitter).Texture, 1536547385) and not checkTrinketSpawn(trinket) then
						artifact = phoenix down (bait)
						found = true
					elseif trinket.Transparency == 1 and trinketFindFirstChildOfClass(PointLight) and trinketFindFirstChildOfClass(PointLight).Color == Color3.fromRGB(132, 255, 0) and not trinketFindFirstChildOfClass(SpecialMesh) and not checkTrinketSpawn(trinket) then
						artifact = ice essence (bait)
						found = true
					elseif trinket.Transparency == 0 and trinketFindFirstChild(coldpart) and trinketFindFirstChildOfClass(SpecialMesh) and trinketFindFirstChildOfClass(SpecialMesh).MeshType == Enum.MeshType.Sphere and not checkTrinketSpawn(trinket) then
						artifact = fairfrozen (bait)
						found = true
					elseif trinket.Transparency ~= 0 and trinket.BrickColor == BrickColor.new(Hot pink) and trinketFindFirstChildOfClass(SpecialMesh) and getId(trinketFindFirstChildOfClass(SpecialMesh).MeshId, 2877143560) and not checkTrinketSpawn(trinket) then
						artifact = rift gem (bait)
						found = true
					end
				elseif trinketIsA(UnionOperation) then
				...
				elseif trinketIsA(MeshPart) then
			...
			if found then
				...
				artifactsLabel.Text = ARTIFACTS ..artifactsFound
				shadow_2.Text = ARTIFACTS ..artifactsFound
			end
		end
		...
		if not syn_io_isfile(artifactServerList.JSON) then
			...
			servers = httpServiceJSONDecode(syn_io_read(artifactServerList.JSON))
			if #servers  1 then
				servers = refresh()
				table.remove(servers, 1)
			end
		...
		syn.queue_on_teleport(loadfile('artifactHunter.lua')())

		local function joinNextServer()
			warn(UPDATING SERVERS...)
			local nextServer = table.remove(servers, 1)
			syn_io_write(artifactServerList.JSON, httpServiceJSONEncode(servers))
			teleportServiceTeleportToPlaceInstance(3016661674, nextServer)
			warn(SERVERS UPDATED!)
		end

		local function teleportFailed(player, teleportResult, errorMessage)
			if player == gameGetService(Players).LocalPlayer then
		...
		teleportService.TeleportInitFailedConnect(teleportFailed)
